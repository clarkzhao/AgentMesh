import type { A2aTask, TaskStoreEntry } from "./types.js";
...
export class TaskStore {
  ...
  set(taskId: string, task: A2aTask): void {
    ...
    if (existing && (existing.task.status.state === "completed" || existing.task.status.state === "failed")) {
  ...
  isTerminal(taskId: string): boolean {
    ...
    return entry.task.status.state === "completed" || entry.task.status.state === "failed";
  }
